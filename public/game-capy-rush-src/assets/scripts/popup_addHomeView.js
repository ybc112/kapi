...
var _ = function (t) {
  ...
  e.prototype.gotoClose = function () {
    c.ManageCtl.uiManage.gotoDestroyUI(f.uiPath.uiName.popup_addHomeView);
  };
  e.prototype.btnClick_addHome = function (t) {
    if (h.default.instance.baseBtnClick(this.node.name + t.target.name)) {
      ...
      window.tt.addShortcut({
        success: function () {
          c.ManageCtl.persistRootNode.showTipsUI("添加桌面成功");
          if (c.ManageCtl.gameData.getFlagData(l.MyConstans.projectName + "_addHome")) {
            c.ManageCtl.persistRootNode.showTipsUI("添加桌面成功");
          } else {
            c.ManageCtl.gameData.setFlagData(l.MyConstans.projectName + "_addHome", {});
            c.ManageCtl.gameData.addDayPropById(l.MyConstans.propId.remove, 1);
            c.ManageCtl.persistRootNode.showTipsUI("恭喜获得消3节龙x1");
            p.statsCtl.sendEventShuShu("activity_event", {
              scene: "addHome",
              lvgrade: c.ManageCtl.gameData.game_lvgrade
            });
          }
          c.ManageCtl.myMsgCtl.emit(l.MyConstans.msg.addHomeDone);
          e.gotoClose();
        },
        fail: function (t) {
  ...
  return a([m, y("ui/popup_addHomeView")], e);
}(u.default);
exports.default = _;